...
This specification describes the **whole product, organized by module**. It is the umbrella spec from which individual feature specs (one per submodule) are derived. The 42 submodules across 15 product modules are listed in [context/backlog.md](../../context/backlog.md); the competitive rationale for each is in [docs/PDR_PerFiApp.md](../../docs/PDR_PerFiApp.md).
...
- **Canada-First & Bilingual** (CAD + time-to-goal; full EN/FR, locale-correct formatting)
- **Test-First (NON-NEGOTIABLE)** (TDD mandatory; no feature ships without tests for happy path and Canadian/bilingual/cross-module edge cases)
...
- **Simplicity & YAGNI** (start simple; complexity must be justified by a real user need)
...
- **FR-X-018 (Template compliance gate)**: The Spec Kit templates (`plan-template.md`, `spec-template.md`, `tasks-template.md`) MUST be brought into compliance with Constitution v2.2.0 — concrete Constitution Check gates in the plan template, mandatory threat-model and money-correctness fields in the spec template, and contract-test / audit / redaction task categories in the tasks template — before the first P1 submodule spec is generated from them. No P1 submodule spec may be authored from a non-compliant template.
- **FR-X-019 (Maximum retention)**: Beyond the 7-day deletion SLA for explicit requests (FR-X-013), the system MUST enforce a maximum retention period for inactive/dormant users who never submit a deletion request: after a defined inactivity window, their financial data MUST be automatically anonymized or deleted (PIPEDA Principle 5 and Quebec Law 25 art. 12 — retain only as long as necessary). The exact inactivity duration is set in the privacy impact assessment during planning.
- **FR-X-020 (Data residency)**: Canadian users' financial data and PII MUST be stored and processed on Canadian-region infrastructure. Any cross-border transfer or processing MUST be explicitly disclosed to the user and covered by an accountability/transfer agreement (PIPEDA accountability for cross-border transfers); the aggregation provider and all subprocessors MUST satisfy this residency constraint. Mirrors the Constitution's Privacy & Compliance data-residency clause.
...
- **FR-CORE-007 (Aggregation token lifecycle & threat model)**: Aggregation access tokens MUST be stored in a dedicated secrets store or hardware-backed key vault — never in an application database column alongside user data — and encrypted with keys the application database cannot access. Tokens MUST be rotated on session invalidation, suspected compromise, a credential/role change (privilege demotion), and on a maximum-age schedule. Each household member who connects an institution MUST obtain and hold an **independent consent grant and token issued under their own identity**; no token may be shared between household members — this is the prerequisite that makes partial revocation implementable. When a household member is removed while other members remain connected to the same institution, the system MUST perform **partial revocation** that invalidates only the departing member's token without disrupting remaining members. The Module 0 spec MUST include a threat model enumerating at minimum: (1) aggregation-token exfiltration, (2) account takeover / credential stuffing against the FinOS authentication surface, and (3) IDOR / horizontal privilege escalation on the household module (per FR-HH-001) — each with mitigations (most aggregation tokens do not self-expire, so explicit revocation paths are mandatory).
...
- **SC-016 (Template compliance)**: 0 P1 submodule specs are generated before `plan-template.md`, `spec-template.md`, and `tasks-template.md` are confirmed compliant with Constitution v2.2.0 (FR-X-018).
